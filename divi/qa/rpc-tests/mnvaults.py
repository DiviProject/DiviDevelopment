...
class MnVaultsTest (MnTestFramework):
  ...
  def connect_to_anchors(self,nodeID):
    for j in range(3,7):
      if nodeID < j:
        connect_nodes(self.nodes[nodeID],j)
  ...
  def setup_network (self, config_line=None, extra_args=[]):
    ...
    for nodeID in range(0,6):
      self.connect_to_anchors(nodeID)
  ...
  def fund_vault (self):
    ...
    bc = self.nodes[2].signmnbroadcast(self.setup[1].broadcast_data)
    assert_equal (self.nodes[1].importmnbroadcast (bc["broadcast_data"]), True)
    self.stop_node (2)
